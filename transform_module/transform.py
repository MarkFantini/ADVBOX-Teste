...
def generate_base_clients(filtered_dfs, name='v_clientes'):
	base_clients = filtered_dfs[name].copy()
	...
	missing_set = set(CLIENTS_FINAL_COLS).difference(BASE_CLIENTS_DICT.keys())

	for col in list(missing_set):
		base_clients[col] = ''
		
	return base_clients


def generate_base_lawsuits(filtered_dfs, name='v_processos'):
	base_lawsuits = filtered_dfs[name]
